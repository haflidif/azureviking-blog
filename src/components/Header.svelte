  ...
  <div class="flex items-center justify-between py-3 sm:py-4 lg:py-5">
    ...
    <div class="flex items-center gap-2 lg:hidden">
      ...
      <button
        ...
        class="p-2 rounded-md text-muted-foreground hover:text-foreground hover:bg-accent transition-all cursor-pointer"
      >
        {#if menuOpen}
          <!-- X icon -->
          <svg
      ...
      <div class="flex flex-col gap-1">
        {#each navLinks as link (link.name)}
          <a
            href={link.href}
            onclick={closeMenu}
            class="px-3 py-2.5 rounded-lg text-sm font-bold uppercase tracking-widest text-muted-foreground hover:bg-accent hover:text-primary transition-all no-underline"
          >
            {link.name}
          </a>
        {/each}
      ...
      <div class="flex items-center gap-3 px-3">
        {#each Object.entries(SITE.social) as [platform, link] (platform)}
          <a
            ...
            rel="noopener noreferrer"
            aria-label={platform}
            class="p-1.5 rounded-md text-muted-foreground hover:text-primary transition-all"
          >
            {#if platform === 'github'}
              <svg
                xmlns="http://www.w3.org/2000/svg"
