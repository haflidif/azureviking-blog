...
- Test with Windows High Contrast and `forced-colors: active` media query.
- Use system color keywords in `@media (forced-colors: active)` when custom overrides are necessary.
- Do not use `forced-color-adjust: none` globally.
